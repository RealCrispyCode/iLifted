                ...
                <div className="flex-1 relative bg-white min-h-0 flex flex-col p-6 text-center">
                  {imageUrl ? (
                    ...
                    <div className="flex-1 w-full h-full flex flex-col items-center justify-between p-6 text-center bg-white">
                      ...
                      <div className="flex items-center gap-2 text-zinc-300 text-[10px] font-bold uppercase tracking-widest pt-4">
                        <span>Generated by iLifted AI</span>
                        {apiStatus && (
                          <div className={cn(
                            "flex items-center gap-1.5 px-2 py-0.5 rounded-full border",
                            apiStatus === 'gemini' ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" :
                            apiStatus === 'fallback' ? "bg-amber-500/10 border-amber-500/20 text-amber-400" :
                            "bg-red-500/10 border-red-500/20 text-red-400"
                          )}>
                            <div className={cn(
                              "w-1 h-1 rounded-full animate-pulse",
                              apiStatus === 'gemini' ? "bg-emerald-400" :
                              apiStatus === 'fallback' ? "bg-amber-400" :
                              "bg-red-400"
                            )} />
                            {apiStatus === 'gemini' ? 'Gemini Pro' : apiStatus === 'fallback' ? 'Pollinations.ai' : 'Error'}
                          </div>
                        )}
                      </div>
                    </div>
